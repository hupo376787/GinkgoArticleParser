...
namespace GinkgoArticleParser.Helpers
{
    public class DatabaseHelper
    {
        ...
        public async static Task<bool> ExportDatabaseAsync()
        {
            ...
            // 导出文件的新名称（添加时间戳避免覆盖）
            string destinationFileName = $"History-{DateTime.Now:yyyyMMdd-HHmmss}.db3";
            ...
            try
            {
                ...
                // 可以添加一个提示
                await ToastHelper.ShowToast($"数据库已保存到: {destinationPath}");
                return true;
            ...
            catch (Exception ex)
            {
                ...
                await ToastHelper.ShowToast($"发生错误: {ex.Message}");
                return false;
            }
        }
...
#if WINDOWS || MACCATALYST
                // 可以添加一个提示
                await ToastHelper.ShowToast($"导入成功，数据库已从 {sourcePath} 导入并覆盖到 {targetDbPath}");
#endif
